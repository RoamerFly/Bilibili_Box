...
export interface AiSummaryPromptPreview { system_prompt: string; instruction: string; user_prompt: string; }
...
export function buildAiModelsRequest(providerId: string, provider?: { base_url: string; kind: string; timeout_secs: number }) { return provider ? { request: { provider_id: providerId, base_url: provider.base_url.trim(), kind: provider.kind, timeout_secs: provider.timeout_secs } } : { request: { provider_id: providerId } }; }
...
export function AiSettingsPanel({ onFeedback, onSettingsSaved }: AiSettingsPanelProps) {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void invoke<AiSummaryPromptPreview>("preview_ai_summary_prompt", { request: { prompt_template: settings.prompt_template || DEFAULT_AI_PROMPT_TEMPLATE } })
        .then((preview) => { if (!cancelled) setPromptPreview(preview); })
        .catch(() => { if (!cancelled) setPromptPreview(null); });
    }, 200);
  ...
  async function listModels(provider: AiProviderSettings) {
    ...
    if (!provider.base_url.trim()) { setModelErrors((current) => ({ ...current, [id]: mapAiModelsError("AI_MODELS_ENDPOINT_INVALID: AI 服务地址无效") })); return; }
...
const textareaStyle: React.CSSProperties = { ...inputStyle, width: "100%", minHeight: 96, resize: "vertical", lineHeight: 1.55 };
const previewContainerStyle: React.CSSProperties = { display: "grid", gap: 6, marginTop: 14, paddingTop: 12, borderTop: "1px solid var(--color-bg-subtle)" };
const previewTitleStyle: React.CSSProperties = { fontSize: 12.5, fontWeight: 700, color: "var(--color-text-secondary)" };
const previewPreStyle: React.CSSProperties = { margin: 0, padding: "12px 14px", borderRadius: 9, border: "1.5px solid var(--color-border)", backgroundColor: "var(--color-bg)", color: "var(--color-text-secondary)", fontSize: 11.5, lineHeight: 1.55, fontFamily: "ui-monospace, SFMono-Regular, Menlo, Consolas, monospace", whiteSpace: "pre-wrap", wordBreak: "break-word", maxHeight: 320, overflowY: "auto" };
