...
if [[ "$#" -gt 0 ]]; then
    echo "ERROR: Usage: ./build-linux.sh [--refresh-icons]" >&2
    exit 1
fi
...
copy_runtime_tool() {
    ...
    shopt -s nullglob
    runtime_libraries=("$tool_dir"/*.so "$tool_dir"/*.so.*)
    shopt -u nullglob
...
if $REFRESH_ICONS; then
    ...
    [[ -f "src-tauri/icons/icon.png" ]] ||
        fail "Linux icon was not found. Run ./build-linux.sh --refresh-icons."
fi
